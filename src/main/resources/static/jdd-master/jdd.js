...
var subtitle1 = $('<div id="reportsubTitle1"><h2>Legacy Response: </h2></div>');
 report.append(subtitle1);
 var subtitle2 = $('<div id="reportsubTitle2"><h2>Microservice Response:</h2></div>');
 report.append(subtitle2);
    },
    ...
    compare: function () {
         ...
         */
        var leftValid = jdd.validateInput(store.left, jdd.LEFT);
        var rightValid = jdd.validateInput(store.right, jdd.RIGHT);
        ...
        var left = JSON.parse(store.left);
        var right = JSON.parse(store.right);
...
const getTransactionData = function(transactionId) {
	console.log("fetching data for ", transactionId)
   fetch('http://localhost:9000/rediscacheDetails')
  .then(response => response.json())
  .then(data =>{
	store.left = JSON.stringify(data.legacyResponse, null, 2)
	store.right = JSON.stringify(data.microserviceResponse, null, 2)
	 jdd.compare();
	/*load.data({
		left: JSON.stringify(data.legacyResponse, null, 2),
		right: JSON.stringify(data.microserviceResponse, null, 2)
	})*/
});
  }
...
jQuery(document).ready(function () {
    ...
    if (jdd.getParameterByName('left')) {
	    store.left = jdd.getParameterByName('left')
       // $('#textarealeft').val(jdd.getParameterByName('left'));
    }

    if (jdd.getParameterByName('right')) {
	store.right = jdd.getParameterByName('right')
